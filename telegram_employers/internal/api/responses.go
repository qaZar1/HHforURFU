...
func (api *APIResponses) GetResponsesByChatIDEmployer(chatIdEmployer int64) ([]models.Responses, error) {
	endpoint := "/responses/chat_id_employer/%d/get"

	resp, err := api.client.R().SetBasicAuth("dev", "test").Get(fmt.Sprintf(endpoint, chatIdEmployer))
	if err != nil {
...
func (api *APIResponses) UpdateResponse(vacancyId int64, setValues models.Responses) (bool, error) {
	endpoint := "/responses/%d/update"

	resp, err := api.client.R().SetBody(setValues).SetBasicAuth("dev", "test").Put(fmt.Sprintf(endpoint, vacancyId))
	if err != nil {
		return false, err
	}
